...
function ResumeBuilder() {
  ...
  const handleSaveDraft = async () => {
    if (!resumeData.personalInfo.nome) {
      toast.error('Por favor, preencha pelo menos seu nome antes de salvar');
      return;
    }
  ...
  const handleAIGenerate = async () => {
    if (!resumeData.personalInfo.jobTitle || !resumeData.personalInfo.industry) {
      toast.error('Por favor, preencha o título do trabalho e a indústria primeiro');
      return;
    }
    ...
    try {
      ...
      toast.success('Conteúdo do currículo gerado com sucesso!');
    } catch (error) {
      console.error('Error generating resume:', error);
      toast.error('Falha ao gerar conteúdo do currículo');
    } finally {
      setGenerating(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold mb-6">Detalhes do Currículo</h2>
            <div className="space-y-6">
              ...
              >
                <FaMagic /> {generating ? 'Gerando...' : 'Gerar com IA'}
              </button>
            </div>
          </div>
          ...
          {/* Template Selection */}
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h3 className="text-lg font-semibold mb-4">Selecionar Modelo</h3>
            <select
          ...
          <div className="flex gap-4">
            ...
            >
              <FaDownload /> {loading ? 'Exporting...' : 'Baixar PDF'}
            </button>
            <button
            ...
            >
              ...
              {savingDraft ? 'Saving...' : 'Salvar Rascunho'}
            </button>
          </div>
        </div>
