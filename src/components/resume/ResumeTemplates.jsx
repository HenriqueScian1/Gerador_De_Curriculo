      ...
      return (
        <DefaultTemplate>
          ...
          <div className="border-b-2 border-gray-300 pb-4 mb-6">
            <h1 className="text-4xl font-bold text-gray-800">{personalInfo.nome || 'Seu nome'}</h1>
            <div className="text-gray-600 mt-2">
              <p>{personalInfo.email || 'email@exemplo.com'} | {personalInfo.telefone || 'Número de telefone'}</p>
              <p>{personalInfo.localizacao || 'Localização'}</p>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {experience.map((exp, index) => (
              <div key={index} className="mb-4">
                ...
                <p className="text-gray-600">{exp.company || 'Empresa'} | {exp.duration || 'Duração'}</p>
          ...
          <div className="mb-6">
            <div className="flex items-center mb-3">
              ...
              <h2 className="text-2xl font-semibold text-gray-800">Educação</h2>
            </div>
            {education.map((edu, index) => (
              <div key={index} className="mb-4">
                <h3 className="text-lg font-semibold">{edu.degree || 'Grau'}</h3>
                <p className="text-gray-600">{edu.school || 'Escola'} | {edu.duration || 'Duração'}</p>
          ...
          <div className="mb-6">
            ...
            {projects.map((project, index) => (
              <div key={index} className="mb-4">
                <h3 className="text-lg font-semibold">{project.name || 'Nome do Projeto'}</h3>
                <p className="text-gray-600">{project.technologies || 'Tecnologias Utilizadas'}</p>
      ...
      return (
        <DefaultTemplate className="font-serif">
          ...
          <div className="text-center border-b-2 border-gray-800 pb-4 mb-8">
            <h1 className="text-5xl font-bold text-gray-900 mb-2">{personalInfo.nome || 'Seu Nome'}</h1>
            <div className="flex justify-center items-center space-x-4 text-gray-700">
              ...
              <span className="flex items-center"><FaPhone className="mr-1" /> {personalInfo.telefone}</span>
              <span className="flex items-center"><FaMapMarkerAlt className="mr-1" /> {personalInfo.localizacao}</span>
            </div>
          </div>

          {/* Professional Summary */}
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 border-b border-gray-400 mb-3">Resumo Profissional</h2>
            <p className="text-gray-700 leading-relaxed">{personalInfo.objetivo || 'Resumo profissional vai aqui'}</p>
          </div>

          {/* Experience */}
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 border-b border-gray-400 mb-3">Experiência</h2>
            {experience.map((exp, index) => (
          ...
          {/* Skills */}
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 border-b border-gray-400 mb-3">Habilidades</h2>
      ...
      return (
        <DefaultTemplate className="font-sans">
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl font-light text-gray-900 mb-2">{personalInfo.nome || 'Seu Nome'}</h1>
            <div className="text-gray-600 space-x-2">
              <span>{personalInfo.email}</span>
              <span>•</span>
              <span>{personalInfo.telefone}</span>
              <span>•</span>
              <span>{personalInfo.localizacao}</span>
            </div>
          </div>

          {/* Experiência */}
          <div className="mb-8">
            <h2 className="text-lg uppercase tracking-wider text-gray-500 mb-4">Experiência</h2>
            {experience.map((exp, index) => (
              <div key={index} className="mb-6">
        ...
        <DefaultTemplate className="bg-gradient-to-br from-indigo-50 to-white">
          ...
          <div className="relative pb-8 mb-8">
            ...
            <div className="relative text-center py-8">
              <h1 className="text-5xl font-bold text-white mb-2">{personalInfo.nome || 'Seu Nome'}</h1>
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-indigo-600 mb-4 flex items-center">
              <FaBriefcase className="mr-2" /> Experiência
            </h2>
            {experience.map((exp, index) => (
