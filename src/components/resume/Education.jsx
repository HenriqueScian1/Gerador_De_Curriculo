...
export default function Education({ education, onChange }) {
  ...
  const generateAIContent = async (index) => {
    ...
    try {
      ...
      console.error('Erro ao gerar descrição de educação:', error);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="ex., Bacharel em Ciências"
              />
            </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                disabled={loading}
              >
                <FaMagic /> {loading ? 'Gerando...' : 'Gerar com IA'}
              </button>
            </div>
            <textarea
              ...
              placeholder="Descreva suas conquistas acadêmicas, cursos relevantes, etc."
            />
          </div>
        </div>
      ))}
